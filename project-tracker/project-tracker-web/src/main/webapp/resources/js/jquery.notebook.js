...
(function($, d, w) {
	...
	/*
	 * This module deals with the CSS transforms. As it is not possible to
	 * easily combine the transform functions with JavaScript this module
	 * abstract those functions and generates a raw transform matrix, combining
	 * the new transform with the others that were previously applied to the
	 * element.
	 */
	var unwrappedText;
	var transform = (function() {
		...
		var getPreviousTransforms = function(elem) {
			return elem.css('-webkit-transform') || elem.css('transform')
					|| elem.css('-moz-transform') || elem.css('-o-transform')
		...
		var buildTransformString = function(matrix) {
			return 'matrix(' + matrix[0] + ', ' + matrix[1] + ', ' + matrix[2]
	...
	var isMac = w.navigator.platform == 'MacIntel', mouseX = 0, mouseY = 0, cache = {
		command : false,
		shift : false,
		isSelecting : false
	}, modifiers = {
		...
		keyboard : {
			...
			isModifier : function(e, callback) {
				var key = e.which, cmd = modifiers[key];
				if (cmd) {
			...
			isBackspace : function(e, callback) {
				if (e.which === 8) {
					callback();
				}
			},
			isDelete : function(e, callback) {
				if (e.which === 46) {
					callback();
				}
			}
		...
		updatePos : function(editor, elem) {
			var sel = w.getSelection(), range = sel.getRangeAt(0), boundary = range
					.getBoundingClientRect(), bubbleWidth = elem.width(), bubbleHeight = elem
					.height(), offset = editor.offset().left, pos = {
		...
		/*
		 * Updates the bubble to set the active formats for the current
		 * selection.
		 */
		updateState : function(editor, elem) {
			elem.find('button').removeClass('active');
			var sel = w.getSelection(), formats = [];
		...
		/*
		 * Recursively navigates upwards in the DOM to find all the format tags
		 * enclosing the selection.
		 */
		checkForFormatting : function(currentNode, formats) {
			var validFormats = [ 'b', 'i', 'u', 'h1', 'h2', 'ol', 'ul', 'li',
					'a' ];
		...
		showLinkInput : function(selection) {
			...
			var hasLink = elem.closest('.jquery-notebook')
					.find('button.anchor').hasClass('active');
			elem.unbind('keydown');
		...
		setPlaceholder : function(e) {
			if (/^\s*$/.test($(this).text())) {
				$(this).empty();
				var placeholder = utils.html.addTag($(this), 'div').addClass(
						'placeholder');
		...
		preserveElementFocus : function() {
			var anchorNode = w.getSelection() ? w.getSelection().anchorNode
					: d.activeElement;
			if (anchorNode) {
				var current = anchorNode.parentNode, diff = current !== cache.focusedElement, children = this.children, elementIndex = 0;
				if (current === this) {
					current = anchorNode;
				} else if (unwrappedText != null) {
					if (unwrappedText.parentNode === this) {
						$(unwrappedText).wrap('<div></div>');
						unwrappedText == null;
					}
				}
		...
		keydown : function(e) {
			...
			utils.keyboard.isBackspace(e, function() {
				var eee = e.target;
			});
		...
		keyup : function(e) {
			...
			/*
			 * This breaks the undo when the whole text is deleted but so far it
			 * is the only way that I fould to solve the more serious bug that
			 * the editor was losing the p elements after deleting the whole
			 * text
			 */
			if (/^\s*$/.test($(this).text())) {
				$(this).empty();
		...
		mouseClick : function(e) {
			...
			if ($(this).parent().find('.bubble:visible').length) {
				var bubbleTag = $(this).parent().find('.bubble:visible'), bubbleX = bubbleTag
						.offset().left, bubbleY = bubbleTag.offset().top, bubbleWidth = bubbleTag
						.width(), bubbleHeight = bubbleTag.height();
		...
		commands : {
			...
			ul : function(e) {
				events.commands.insertList(e, 'insertUnorderedList');
			},
			ol : function(e) {
				events.commands.insertList(e, 'insertOrderedList');
			},
			undo : function(e) {
				e.preventDefault();
			...
			insertList : function(e, type) {
				var list, elem = window.getSelection().focusNode.parentNode;
				...
				jQuery.event.trigger('listAddedEvent', [ list, elem ]);
			}
		},
		enterKey : function(e) {
			...
			var anchorNode = w.getSelection() ? w.getSelection().anchorNode
					: d.activeElement;
			if (anchorNode && anchorNode.parentNode === this) {
				unwrappedText = anchorNode;
			}
			events.change.call(this);
		},
		paste : function(e) {
			var elem = $(this), id = 'jqeditor-temparea', range = utils.selection
					.save(), tempArea = $('#' + id);
			if (tempArea.length < 1) {
				...
				tempArea.attr('id', id);
				elem.append(tempArea);
			}
			tempArea.focus();

			setTimeout(function() {
				var clipboardContent = '', paragraphs = tempArea.val().split(
						'\n');
		...
		change : function(e) {
			var contentArea = $('#jquery-notebook-content-'
					+ $(this).attr('data-jquery-notebook-id'));
			contentArea.val($(this).html());
			var content = contentArea.val();
			// var changeEvent = new CustomEvent('contentChange', { 'detail': {
			// 'content' : content }});
	...
	$(this).on("listAddedEvent", function(event, param1, param2) {
	});
